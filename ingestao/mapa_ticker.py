#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""mapa_ticker.py — gera CNPJ(14 digitos) -> [tickers] a partir do FCA da CVM."""
import io
import re
import sys
import json
...
def gerar_mapa(conteudo: bytes) -> dict:
    ...
    col_tk = _achar_coluna(df, "NEGOCIACAO")
    ...
    if col_data:
        ...
        maxd = dff.groupby("_CNPJ")["_DT"].transform("max")
    ...
    for _, r in dff.iterrows():
        ...
        if len(cnpj) != 14:
            continue
        mapa.setdefault(cnpj, set()).add(r["_TK"])
